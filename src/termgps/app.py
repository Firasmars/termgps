#!/usr/bin/env python3
"""
TermGPS - Fast Navigation with Themes

Optimized for speed. Same look, more features.
"""
...
# Themes - simple color sets
THEMES = {
    "matrix": {"fg": "green", "bg": "#000", "hl": "yellow", "rt": "blue", "err": "red"},
    "dark": {"fg": "white", "bg": "#1a1a2e", "hl": "cyan", "rt": "blue", "err": "red"},
    "ocean": {"fg": "cyan", "bg": "#0a1628", "hl": "green", "rt": "blue", "err": "red"},
    "sunset": {"fg": "orange1", "bg": "#1a0a0a", "hl": "yellow", "rt": "orange1", "err": "red"},
}
theme = "matrix"
...
class Radar(Static):
    ...
    def render(self):
        ...
        # YOU marker in YELLOW - aligned with crosshair
        if self.lat:
            label = " ◉ YOU"
        ...
        if self.route and self.lat and len(self.route) > 1:
            ...
            # Draw arrow line from center towards destination
            ...
            if arrow_len > 3:
                ...
                if 0<=end_x<w and 0<=end_y<h:
                    buf[end_y][end_x] = arrow_char
                    col[end_y][end_x] = 'red'
        
        # Next turn marker in RED
...
class ThemePanel(Static):
    def render(self):
        t = THEMES[theme]; txt = Text()
        txt.append("─── THEME ───\n", style=Style(color=t["hl"], bold=True))
        txt.append(f" [{theme}] 1-4 to change\n", style=Style(color=t["fg"]))
        return txt
...
class TermGPS(App):
    ...
    BINDINGS = [
        ...
        Binding("t","track","Track"), Binding("c","clear","Clear"), Binding("n","next","Next"), Binding("p","prev","Prev"),
        Binding("1","t1"), Binding("2","t2"), Binding("3","t3"), Binding("4","t4"),
    ...
    def __init__(self):
        ...
        self.tracking=False; self._sa=False; self._q=""; self._timer=None
    
    def compose(self):
        yield Header()
        self.radar=Radar(); self.dir=DirPanel(); self.turns=TurnsPanel(); self.info=InfoPanel()
        self.thm=ThemePanel(); self.sugs=Sugs(); self.inp=Input(placeholder="Search...")
    ...
    def _refresh(self):
        ...
        if self.route:
            self.info.dist = sum(s["dist"] for s in self.route["steps"][self.cur:])
            self.info.time = self.route["time"] * (self.info.dist / self.route["dist"]) if self.route["dist"] else 0
        
        self.radar.refresh(); self.dir.refresh(); self.turns.refresh(); self.info.refresh(); self.thm.refresh()
    ...
    def _tick(self):
        if self.tracking: self.lat, self.lon, self.info.acc = get_gps(); self._check(); self._refresh()
    ...
    def action_t1(self): global theme; theme = "matrix"; self._refresh(); self.notify("Matrix")
    def action_t2(self): global theme; theme = "dark"; self._refresh(); self.notify("Dark")
    def action_t3(self): global theme; theme = "ocean"; self._refresh(); self.notify("Ocean")
    def action_t4(self): global theme; theme = "sunset"; self._refresh(); self.notify("Sunset")
